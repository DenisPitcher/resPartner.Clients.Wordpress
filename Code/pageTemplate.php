...
class pageTemplate {
	...
	private function __construct() {
		...
		$this->templates = array(
            'resQwestNoSidebar.php'     => __( 'resQwest NoSidebar', $this->plugin_slug ),
	...
	public function view_project_template( $template ) {
       ...
       $hideSidebar = resQwest_get_option('resQwest_hideSidebar');
       if ($hideSidebar != 'on') return $template;

		// If no posts found, return to
		// avoid "Trying to get property of non-object" error
		if ( !isset( $post ) ) return $template;

        if ( $post->virtualPage == 1)
        {
            $virtualTemplate = plugin_dir_path( __FILE__ ) . 'templates/resQwestNoSidebar.php';
            if (file_exists($virtualTemplate))
            {
